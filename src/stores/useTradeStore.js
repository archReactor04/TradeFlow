...
import { buildFingerprint } from '@/lib/trade-utils';
...
export function useTradeStore() {
  ...
  const importTrades = useCallback(async (newTrades) => {
    ...
    const existingBrokerIds = new Set(
      existing.filter(t => t.brokerTradeId).map(t => `${t.brokerTradeId}|${t.accountId ?? ''}`)
    );
    const existingFingerprints = new Set(existing.map(buildFingerprint));

    const toAdd = [];
    let skipped = 0;

    for (const t of newTrades) {
      const brokerKey = t.brokerTradeId ? `${t.brokerTradeId}|${t.accountId ?? ''}` : null;
      const fp = buildFingerprint(t);

      if (brokerKey && existingBrokerIds.has(brokerKey)) { skipped++; continue; }
      if (!brokerKey && existingFingerprints.has(fp))    { skipped++; continue; }

      if (brokerKey) existingBrokerIds.add(brokerKey);
      existingFingerprints.add(fp);

      toAdd.push({ ...t, id: t.id || crypto.randomUUID() });
    }
